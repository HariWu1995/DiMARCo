...
def draw_task(task, width=30, height=12, include_test=False, label=True, bordercols=['#111111ff', '#111111ff'], shortdesc=False):
    ...
    padding_extra = 0.25
    io_gap = 0.4
    ymax = (height - padding - padding_extra - io_gap)/2
    ...
    # If any examples would exceed the height restriction, 
    #   scale their width down and redistribute the space
    ...
    for i, (input_grid, output_grid) in enumerate(examples):
        ...
        # Down arrow
        drawlist.append(drawsvg.Line(x_ptr + input_x/2, 
                                     y_ptr + padding - 0.6, 
                                     x_ptr + input_x/2, 
                                     y_ptr + padding + io_gap - 0.6, 
                                     stroke_width=0.05, stroke='#888888'))
        
        drawlist.append(drawsvg.Line(x_ptr + input_x/2 - 0.15,
        ...
        if i < n_train or include_test == 'all':
            drawlist.append(drawsvg.Use(output_grid, 
                                        x = x_ptr - offset[0] + (allocation[i]+padding-output_x)/2, 
                                        y = y_ptr - offset[1] + io_gap))
        else:
            # Add a question mark
            drawlist.append(drawsvg.Text('?',
                                        x = x_ptr + (allocation[i]+padding)/2,
                                        y = y_ptr + output_y/2 + padding_extra,
                                        font_size=1,
                                        font_family='Anuphan',
                                        font_weight='700',
                                        fill='#333333',
    ...
    for item in drawlist:
        d.append(item)
    d.append(drawsvg.Text(f"Task {task.id}", 
                          x=x_ptr-0.1, 
                          y=y_ptr2+0.1, 
                          font_size=0.3, 
                          font_family='Anuphan', 
